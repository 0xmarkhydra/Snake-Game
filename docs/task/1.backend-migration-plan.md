...
## 10. Flow Chi Tiết Để Move Server

### Flow 10.1 – Chuẩn Bị Backend NestJS
1. **Clone & cài đặt**: đảm bảo thư mục `backend/` cài `pnpm install`.
2. **Cấu hình môi trường**: tạo `.env` (cổng HTTP, DB, Colyseus host/port).
3. **Bật swagger & format response**: confirm interceptor toàn cục đã đăng ký trong `main.ts`.
4. **Khởi tạo module Game**: tạo `GameModule`, `GameService`, `GameGateway`.

### Flow 10.2 – Port Schema/State
1. Copy toàn bộ class trong `server/src/rooms/schema` sang `backend/src/modules/game/rooms/schema`.
2. Giữ nguyên decorator Colyseus (`@type`) và logic helper (`addSegment`, `updateHeadPosition`).
3. Tạo index export để các room mới import thống nhất.
4. Viết unit test nhanh cho `Player.addSegment()` để bảo đảm hoạt động như cũ.

### Flow 10.3 – Port Room Free (snake_game)
1. Tạo file `free-game.room.ts` trong `backend/src/modules/game/rooms`.
2. Sao chép logic `SnakeGameRoom` (onCreate/onJoin/gameLoop/handlers) và chỉnh import sang schema mới.
3. Thay `this.clock` thành clock từ Colyseus API (không đổi).
4. Đăng ký room trong `GameGateway`:
   ```ts
   gameServer.define('snake_game', FreeGameRoom);
   ```
5. Viết e2e test đơn giản: tạo client giả, join room, đảm bảo nhận `welcome` + `initialFoods`.

### Flow 10.4 – Kết Nối Với Client Hiện Tại
1. Cập nhật `ui/src/services/ColyseusClient.ts` để trỏ tới URL backend mới (host/port).
2. Kiểm tra join free room: gameplay (di chuyển, ăn, chết) hoạt động như cũ.
3. Ghi lại các message mới/phát sinh (nếu có) để đảm bảo tương thích.

### Flow 10.5 – Tách Room VIP (giữ logic cũ ở giai đoạn này)
1. Clone `FreeGameRoom` thành `VipGameRoom`.
2. Đăng ký room mới `snake_game_vip` (tạm thời chưa dùng credit).
3. Đảm bảo client có thể chuyển giữa hai room bằng cấu hình `joinOrCreate`.
4. Sau khi confirm hoạt động, ghi chú lại phần sẽ cải tiến trong các sprint tiếp theo (credit, auth).

### Flow 10.6 – Hoàn Tất & Bàn Giao
1. Viết script `pnpm start:game` (hoặc cập nhật `package.json`) để chạy backend Nest + Colyseus.
2. Cập nhật tài liệu README backend với hướng dẫn chạy mới.
3. Đánh dấu TODO trong code nơi cần nâng cấp (credit, auth) cho phase sau.
4. Sau khi xác nhận ổn định, archive thư mục `server/` (để dễ rollback nếu cần).
