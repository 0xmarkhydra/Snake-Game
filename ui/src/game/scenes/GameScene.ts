...
export class GameScene extends Scene {
    ...
    private playerSegmentHistories: Map<
        string,
        Array<{ x: number; y: number }>
    > = new Map(); // Store histories for all players
    ...
    async create() {
        ...
        if (this.physics && this.physics.world) {
            ...
            console.warn(
                "Physics system not available, skipping world bounds setup"
            );
        }
    ...
    update(time: number, delta: number) {
        ...
        if (player && player.alive) {
            ...
            if (this.lastAngle !== undefined) {
                ...
                const effectiveMaxAngleChange = player.boosting
                    ? this.maxAngleChange * 1.5
                    : this.maxAngleChange;
    ...
    private createBackground() {
        ...
        for (let x = 0; x < tilesX; x++) {
            for (let y = 0; y < tilesY; y++) {
                this.add
                    .image(x * bgWidth, y * bgHeight, "background")
                    .setOrigin(0, 0)
                    .setAlpha(0.5);
            }
        }
        ...
        // Add mask layer with background.jpg
        // This layer sits above the base background but below game elements
        const maskTexture = this.textures.get("background-mask");
        const maskWidth = maskTexture.getSourceImage().width;
        const maskHeight = maskTexture.getSourceImage().height;

        // Calculate how many tiles we need for the mask
        const maskTilesX = Math.ceil(this.worldWidth / maskWidth) + 1;
        const maskTilesY = Math.ceil(this.worldHeight / maskHeight) + 1;

        // Create the mask tiles
        for (let x = 0; x < maskTilesX; x++) {
            for (let y = 0; y < maskTilesY; y++) {
                this.add
                    .image(x * maskWidth, y * maskHeight, "background-mask")
                    .setOrigin(0, 0)
                    .setAlpha(1.0) // Fully opaque overlay
                    .setDepth(2); // Above background (0) and grid, below food (4+) and snakes (10+)
            }
        }
    }
    ...
    private createUI() {
        ...
        cardBg.fillGradientStyle(
            0x000033,
            0x000033, // Dark blue at top
            0x000022,
            0x000022, // Darker blue at bottom
            1,
            1,
            1,
            1
        ...
        titleBg.fillRoundedRect(0, 0, cardWidth, 30, {
            tl: 10,
            tr: 10,
            bl: 0,
            br: 0,
        ...
        const title = this.add
            .text(cardWidth / 2, 15, "PLAYER STATS", {
                fontFamily: "Arial",
                fontSize: "16px",
                fontStyle: "bold",
                color: "#ffffff",
    ...
    private createLeaderboard() {
        ...
        for (let i = 0; i < 5; i++) {
            const entry = this.add
                .text(
                    -80,
                    -50 + i * 35, // Increased vertical spacing from 30 to 35
                    `${i + 1}. ---`,
                    {
                        fontFamily: "Arial",
                        fontSize: "16px",
    ...
    private createDeathOverlay() {
        ...
        const deathText = this.add
            .text(width / 2, height / 2 - 100, "YOU DIED", {
                fontFamily: "Arial",
                fontSize: "64px",
                color: "#ff0000",
                stroke: "#000000",
        ...
        // Respawn button
        this.respawnButton = this.add
            .text(width / 2, height / 2 + 100, "RESPAWN", {
                fontFamily: "Arial",
                fontSize: "32px",
                color: "#ffffff",
        ...
        this.menuButton = this.add
            .text(width / 2, height / 2 + 180, "BACK TO MENU", {
                fontFamily: "Arial",
                fontSize: "24px",
                color: "#ffffff",
        ...
        this.gameState.players.forEach((playerData: any, id: string) => {
            ...
            if (!snake) {
                ...
                for (let i = 0; i < initialSegments; i++) {
                    ...
                    const finalTexture = textureExists
                        ? texture
                        : isHead
                        ? "snake-head"
                        : "snake-body";
            ...
            if (actualSegmentCount < targetSegmentCount) {
                ...
                for (let i = actualSegmentCount; i < targetSegmentCount; i++) {
                    ...
                    // Get position of last segment (tail) to spawn new segment there
                    const children = snake.getChildren();
                    let lastSegmentX = 0;
                    let lastSegmentY = 0;

                    // Find the last actual segment (skip glow)
                    for (let j = children.length - 1; j >= 0; j--) {
                        const child = children[j] as Phaser.GameObjects.Image;
                        if (
                            child &&
                            child.texture &&
                            child.texture.key !== "glow"
                        ) {
                            lastSegmentX = child.x;
                            lastSegmentY = child.y;
                            break;
                        }
                    }

                    // Create new segment at tail position (not at 0,0)
            ...
            if (headObj) {
                ...
                const newX =
                    currentX + (headPosition.x - currentX) * lerpFactor;
                const newY =
                    currentY + (headPosition.y - currentY) * lerpFactor;
                ...
                headObj.setRotation(
                    Phaser.Math.DegToRad(playerData.angle + 90)
                );
                ...
                // Head is always brightest! Apply bright tint
                const headBrightnessTint =
                    Phaser.Display.Color.Interpolate.ColorWithColor(
                        ...
                        100,
                        40 // 40% white mixed in for head (brightest)
                    );
                headObj.setTint(
                    Phaser.Display.Color.GetColor(
                        headBrightnessTint.r,
                        headBrightnessTint.g,
                        headBrightnessTint.b
                    )
                );
                ...
                if (playerData.boosting) {
                    headObj.setAlpha(
                        0.8 + Math.sin(this.time.now * 0.01) * 0.2
                    ); // Pulsing effect
                    ...
                    if (glowObj) {
                        glowObj.setAlpha(
                            0.5 + Math.sin(this.time.now * 0.01) * 0.2
                        );
                ...
                for (let i = 1; i < children.length; i++) {
                    ...
                    const historyIndex = Math.min(
                        Math.floor(
                            (i - 1) * (this.segmentSpacing / playerData.speed)
                        ),
                    ...
                    if (segmentHistory[historyIndex]) {
                        ...
                        // All segments have the same size now!
                        segmentObj.setScale(baseScale);

                        // Add subtle pulsing effect to segments
                        const pulseAmount =
                            0.05 * Math.sin(this.time.now * 0.005 + i * 0.5);
                        segmentObj.setScale(baseScale * (1 + pulseAmount));

                        // Gradient brightness effect: bright at head, darker toward tail
                        // Calculate brightness based on segment position (0 = head is brightest)
                        const totalSegments = children.length - 1; // Exclude glow
                        const segmentIndex = i - 1; // Subtract 1 because i starts at 1 (0 is head)
                        const brightnessRatio =
                            1 - (segmentIndex / totalSegments) * 0.5; // From 1.0 to 0.5

                        // Apply brightness via tint (brighter = closer to white)
                        const brightnessTint =
                            Phaser.Display.Color.Interpolate.ColorWithColor(
                                Phaser.Display.Color.ValueToColor(
                                    parseInt(color.replace("#", "0x"))
                                ),
                                Phaser.Display.Color.ValueToColor(0xffffff),
                                100,
                                brightnessRatio * 30 // 0-30% white mixed in
                            );
                        segmentObj.setTint(
                            Phaser.Display.Color.GetColor(
                                brightnessTint.r,
                        ...
                        // Also apply subtle alpha variation for a more organic look
                        const alphaVariation =
                            0.05 * Math.sin(this.time.now * 0.003 + i * 0.3);
                        segmentObj.setAlpha(brightnessRatio + alphaVariation);
                    }
                }
            }
        });
    ...
    private updateFoods() {
        ...
        this.foods.forEach((foodSprite, foodId) => {
            if (!this.gameState.foods.has(foodId)) {
                ...
                if (glow) {
                    // If it's a container, destroy it properly
                    if (glow.destroy) {
                        glow.destroy();
                    }
                }
        ...
        this.gameState.foods.forEach((foodData: any, foodId: string) => {
            ...
            if (!this.foods.has(foodId)) {
                ...
                if (foodSprite) {
                    // Get previous base position
                    const prevBaseX = foodSprite.getData("baseX");
                    const prevBaseY = foodSprite.getData("baseY");

                    // Check if server position changed (food respawned in new location)
                    const hasServerMoved =
                        prevBaseX === undefined ||
                        prevBaseY === undefined ||
                        Math.abs(prevBaseX - position.x) > 10 ||
                        Math.abs(prevBaseY - position.y) > 10;

                    // Update base position
                    foodSprite.setData("baseX", position.x);
                    foodSprite.setData("baseY", position.y);

                    // Only reset visual position if server moved the food significantly
                    if (hasServerMoved) {
                    ...
                    // Otherwise, keep the current position (affected by attraction)

                    // Update texture if value changed
                    if (
                        (value > 1 &&
                            foodSprite.texture.key !== "special-food") ||
                        (value === 1 && foodSprite.texture.key !== "food")
                    ) {
                        foodSprite.setTexture(
                            value > 1 ? "special-food" : "food"
                        );

                        // Note: We now always have glow, so no need to add/remove it based on value
                        // Just handle texture changes
                    }
                }
            }
        });
    ...
    private createFoodSprite(
        id: string,
        x: number,
        y: number,
        value: number
    ): Phaser.GameObjects.Image {
        // Select random food texture for normal food, special texture for special food
        let texture: string;
        let glowColor: number;

        if (value > 1) {
            texture = "special-food";
            glowColor = 0xffff00; // Golden glow
        } else {
            // Random color for normal food
            const foodTextures = [
                "food",
                "food-blue",
                "food-green",
            ...
            const randomIndex = Math.floor(Math.random() * foodTextures.length);
            texture = foodTextures[randomIndex];

            // Match glow color to food color
            const glowColors = [
                0xff4444, 0x4444ff, 0x44ff44, 0xffff44, 0xff44ff, 0xff8844,
                0x44ffff,
            ];
            glowColor = glowColors[randomIndex];
        }
        ...
        // Store the food value and base position for reference
        foodSprite.setData("value", value);
        foodSprite.setData("baseX", x);
        foodSprite.setData("baseY", y);

        // Add glow effect for ALL food (not just special)
        // Create multiple glow layers for stronger effect
        const glowContainer = this.add.container(x, y);
        glowContainer.setDepth(3); // Below food

        // Outer glow (largest, most transparent)
        const outerGlow = this.add.circle(0, 0, 20, glowColor, 0.2);
        glowContainer.add(outerGlow);

        // Middle glow
        const midGlow = this.add.circle(0, 0, 15, glowColor, 0.3);
        glowContainer.add(midGlow);

        // Inner glow (smallest, most opaque)
        const innerGlow = this.add.circle(0, 0, 10, glowColor, 0.4);
        glowContainer.add(innerGlow);

        // Store reference to the glow container
        foodSprite.setData("glow", glowContainer);

        // Add pulsing animation to all glows
        this.tweens.add({
            targets: [outerGlow, midGlow, innerGlow],
            scale: { from: 1.0, to: 1.3 },
            alpha: { from: 0.4, to: 0.7 },
            duration: value > 1 ? 600 : 1000, // Special food pulses faster
            yoyo: true,
            repeat: -1,
            ease: "Sine.easeInOut",
        });
        ...
        // Add a gentle bobbing animation for visual appeal
        this.tweens.add({
            targets: foodSprite,
            y: y - 3,
            duration: 1500,
    ...
    private updateLeaderboard() {
        ...
        if (this.leaderboardPanel) {
            ...
            this.leaderboardPanel = this.add.container(
        ...
        const titleBg = this.add.graphics();
        titleBg.fillStyle(0x3333ff, 0.8);
        titleBg.fillRoundedRect(-bgWidth / 2, 0, bgWidth, 40, {
            tl: 10,
            tr: 10,
            bl: 0,
            br: 0,
        ...
        const killsHeader = this.add.text(
            -bgWidth / 2 + 190,
            headerY,
            "KILLS",
            {
        ...
        topPlayers.forEach((player, index) => {
            ...
            const rank = this.add
                .text(-bgWidth / 2 + 20, rowY, rankText, {
                    fontFamily: "Arial",
                    fontSize: "14px",
                    color: rankColor,
            ...
            const name = this.add
                .text(-bgWidth / 2 + 65, rowY, nameText, {
                    fontFamily: "Arial",
                    fontSize: "14px",
                    color: nameColor,
                    fontStyle: isCurrentPlayer ? "bold" : "normal",
            ...
            const score = this.add
                .text(-bgWidth / 2 + 140, rowY, `${player.score}`, {
                    fontFamily: "Arial",
                    fontSize: "14px",
                    color: nameColor,
                    fontStyle: isCurrentPlayer ? "bold" : "normal",
            ...
            const kills = this.add
                .text(-bgWidth / 2 + 190, rowY, `${player.kills}`, {
                    fontFamily: "Arial",
                    fontSize: "14px",
                    color: nameColor,
                    fontStyle: isCurrentPlayer ? "bold" : "normal",
        ...
        if (currentPlayerIndex !== -1) {
            if (this.playerRankText) {
                this.playerRankText.setText(
                    `Rank: ${currentPlayerIndex + 1}/${players.length}`
                );
            }
        }
    }
    ...
    private showDeathOverlay() {
        ...
        if (player) {
            ...
            if (scoreText && scoreText instanceof Phaser.GameObjects.Text) {
                ...
                const newScoreText = this.add
                    .text(width / 2, height / 2, `Score: ${player.score}`, {
                        fontFamily: "Arial",
                        fontSize: "32px",
                        color: "#ffffff",
                        stroke: "#000000",
    ...
    private updateCamera() {
        ...
        if (player && player.alive && player.headPosition) {
            ...
            const newX =
                currentX +
                (this.targetCameraX - currentX) * this.cameraLerpFactor;
            const newY =
                currentY +
                (this.targetCameraY - currentY) * this.cameraLerpFactor;
        ...
        this.foods.forEach((foodSprite, foodId) => {
            ...
            if (angleDiff <= attractionConeAngle / 2) {
                ...
                const alignmentFactor =
                    1 - angleDiff / (attractionConeAngle / 2);
                const distanceFactor = 1 - distance / attractionDistance;
                const attractionForce =
                    attractionStrength * alignmentFactor * distanceFactor;
                ...
                // Update glow position to follow food
                const glow = foodSprite.getData("glow");
                if (glow) {
                    glow.setPosition(foodSprite.x, foodSprite.y);
                }
                ...
                if (newDistance < eatDistance) {
                    // Get and destroy glow container if it exists before hiding the food
                    const glow = foodSprite.getData("glow");
                    ...
                    this.addEatEffect(
                        foodSprite.x,
                        foodSprite.y,
                        foodSprite.getData("value") || 1
                    );

                    // Send message to server that food was eaten, including current positions
                    console.log(
                        `Sending eatFood message for food ${foodId}, distance: ${newDistance}`
                    );
                ...
                if (key === this.playerId) {
                    ...
                    if (currentSnake) {
                        ...
                        if (targetSegmentCount > currentSegmentCount) {
                            for (
                                let i = currentSegmentCount;
                                i < targetSegmentCount;
                                i++
                            ) {
                                // Create new segment
                                const newSegment = this.add.image(
    ...
    private showKillNotification(
        killerSessionId: string,
        killedSessionId: string
    ) {
        console.log(
            `showKillNotification called with killer: ${killerSessionId}, killed: ${killedSessionId}`
        );

        // Get player names or use session IDs if names aren't available
        const killerName =
            this.getPlayerName(killerSessionId) ||
            `Player ${killerSessionId.substr(0, 4)}`;
        const killedName =
            this.getPlayerName(killedSessionId) ||
            `Player ${killedSessionId.substr(0, 4)}`;

        console.log(
            `Notification text: ${killerName} eliminated ${killedName}!`
        );
        ...
        background.fillGradientStyle(
            0x990000,
            0x990000, // Red gradient at top
            0x330000,
            0x330000, // Darker red at bottom
            1,
            1,
            1,
            1
        ...
        const skull = this.add
            .image(-bgWidth / 2 + 30, 0, "food") // Replace with skull icon if available
            .setTint(0xff0000)
            .setScale(1.5);
        container.add(skull);
        ...
        const killerText = this.add
            .text(-bgWidth / 2 + 60, 0, killerName, {
                fontFamily: "Arial",
                fontSize: "18px",
        ...
        const eliminatedText = this.add
            .text(
                -bgWidth / 2 + 60 + killerWidth + 5,
                0,
        ...
        this.time.delayedCall(3500, () => {
            ...
            this.tweens.add({
                ...
                onComplete: () => {
                    container.destroy();
                    this.killNotifications = this.killNotifications.filter(
                        (n) => n !== container
                    );
