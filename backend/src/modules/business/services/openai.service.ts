...
export class OpenAIService {
  ...
  streamCompletion(question: string): Observable<string> {
    return new Observable<string>((observer) => {
      ...
      try {
        console.log(
          `✅ [OpenAIService] [streamCompletion] question:`,
          question,
        );

        if (!this.apiKey) {
          console.log(
            `🔴 [OpenAIService] [streamCompletion] API key is missing`,
          );
        ...
        console.log(
          `🔍 [OpenAIService] [streamCompletion] Request data:`,
          JSON.stringify(data),
        );
        ...
        axios
          ...
          .then((response) => {
            console.log(
              `✅ [OpenAIService] [streamCompletion] Stream connection established`,
            );
            ...
            response.data.on('data', (chunk: Buffer) => {
              try {
                  ...
                  if (line.startsWith('data: ')) {
                    ...
                    try {
                      ...
                      if (json.choices && json.choices[0]?.delta?.content) {
                        ...
                        console.log(
                          `🔄 [OpenAIService] [streamCompletion] Chunk received: "${contentChunk}"`,
                        );
                      ...
                      // Skip invalid JSON
                      console.log(
                        `⚠️ [OpenAIService] [streamCompletion] Invalid JSON:`,
                        jsonStr,
                      );
                    }
                  }
                ...
                console.log(
                  `🔴 [OpenAIService] [streamCompletion] Error processing chunk:`,
                  error,
                );
              }
            });
...
----- RESPONSE END -----`);
          ...
          .catch((error) => {
            ...
            if (axios.isCancel(error)) {
              console.log(
                `ℹ️ [OpenAIService] [streamCompletion] Request was canceled by client`,
              );
              if (completeResponse) {
...
----- PARTIAL RESPONSE END -----`);
              ...
              console.log(
                `🔴 [OpenAIService] [streamCompletion] Axios error:`,
                {
        ...
        return () => {
          console.log(
            `✅ [OpenAIService] [streamCompletion] Stream closed by client`,
          );
          if (completeResponse) {
